...
export default {
  async fetch(request, env, ctx) {
    ...
    // 非 /api 请求：交给 Workers Assets 托管静态前端
    ...
    const isReadPublic =
      method === "GET" &&
      (head === "categories" || head === "bookmarks" || head === "icons" || head === "icon");
    const needsAuth =
      ["POST", "PUT", "PATCH", "DELETE"].includes(method) ||
      ["backup", "restore", "data", "icons"].includes(head) ||
      (head === "password" && method === "POST");
    ...
    if (head === "backup" && method === "GET") {
      const [cats, bms, icons] = await Promise.all([getCats(env), getBms(env), getIcons(env)]);
      return json({
        version: 2,
